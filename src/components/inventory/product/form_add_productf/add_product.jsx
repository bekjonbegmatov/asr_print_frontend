...
function Addproduct(props) {
    ...
    useEffect(() => {
        fetch('http://127.0.0.1:8000brlik/all')
    ...
    function submit() {
        let url = "http://127.0.0.1:8000inventory/create"
        if (check()) {
            fetch(url, {
                method: "POST",
                headers: {
    ...
    function add_birlik() {
        let url = "http://127.0.0.1:8000brlik/create"
        if (birlik.length != 0) {
            fetch(url, {
                method: "POST",
                headers: {
        ...
        <div className='top-50 start-50'>
            <div>
                <form id='form'>
                    ...
                    {/* <input type="number" className='form form-control' placeholder='Расход' onChange={get_percent} /> */}
                    <br />
